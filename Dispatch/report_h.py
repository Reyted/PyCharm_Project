...
def creat_report(url_attach,url_tpl,url_img,city):
    wb = xw.Book(
        f'{url_attach}{city}LTE&NR测试分析优化问题点12月.xlsx')
    ...
    for data in data_list:
        ...
        dt=str(data[10]).split('、')
        dtstr=''
        for i in dt:
            if len(i)>2:
                dtstr+=i.strip().replace('\n2','')
        context = {
            'system': data[2],
            'position': pos,
            'question_type': type,
...
def center_insert_img(doc,img,img1):
    """插入图片"""
    for paragraph in doc.paragraphs:
        ...
        if 'img1' in paragraph.text:
            ...
            run.add_break()
            # 添加图片并指定大小
            run.add_picture(img1, width=Inches(6.2))
...
def save_img_to_doc(img,img1,url_tpl,title_lst:list):
    """把图片保存到doc文件中的指定位置"""
    ...
    # 插入图片居中
    center_insert_img(document,img,img1)
    # 保存结果文件
...
def main(url_img:str,url_tpl,title_lst:list,city:str):
    ...
    img1 = f'{url_img}{city}/{title_lst[4]}r.png'
    save_img_to_doc(img,img1,url_tpl,title_lst)

if __name__ == '__main__':
    url_attach='C:/Users/24253/Desktop/每月例行工作/12月工作内容/12月工单报告/12月工单报告附件/'
    url_tpl='C:/Users/24253/Desktop/每月例行工作/12月工作内容/12月派单回单/回单/'
    url_img='C:/Users/24253/Desktop/每月例行工作/12月工作内容/12月派单回单/image/'
    city_lst=['昌都市类乌齐县','昌都至类乌齐国道']
    for city in city_lst:
        creat_report(url_attach,url_tpl,url_img,city)
    #creat_report(url_attach, url_tpl, url_img, city_lst[3])